...
function isPointWithinRectangle( p1, boundingRect )

	if p1.x > boundingRect.leftBorderX and p1.x < boundingRect.rightBorderX and p1.y < boundingRect.topBorderY and p1.y > boundingRect.bottomBorderY then
		return true
	end
		return false
end

function isPointWithinEntity( p1, entity )
	local bounds = entity:GetBounds()
	local origin = entity:GetAbsOrigin()
	if p1.x > (origin.x + bounds.Mins.x) and p1.x < (origin.x + bounds.Maxs.x) and p1.y > (origin.y + bounds.Mins.y) and p1.y < (origin.y + bounds.Maxs.y) then
		return true
	end
		return false
end
